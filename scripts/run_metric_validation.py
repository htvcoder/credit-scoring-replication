"""Run the Phase 4 metric-validation harness and write a non-publishable artifact."""
...
from creditrep.artifacts.metric_validation import create_metric_validation_artifact
from creditrep.checksums import get_dataset_checksum
from creditrep.config.metric_validation import load_metric_validation_config
...
def main() -> int:
    ...
    parser.add_argument("--config", required=True, help="Path to metric validation YAML config.")
    parser.add_argument("--dry-run", action="store_true", help="Validate config and split contract without writing artifacts.")
    args = parser.parse_args()
    repo_root = find_repo_root()
    try:
        config = load_metric_validation_config(args.config, repo_root=repo_root)
                ...
                json.dumps(
                    {
                        ...
                        "dry_run": True,
                        "metrics": [metric.metric_id for metric in config.metrics],
                        "nested_cv_hash": nested_cv.nested_cv_hash,
                        "outer_folds": len(nested_cv.outer_folds),
                    },
                    sort_keys=True,
                )
            )
        ...
        json.dumps(
            {
                ...
                "metrics": manifest["evaluation"]["metric_ids"],
