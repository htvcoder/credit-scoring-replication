...
| MLP-1 | replication baseline | Paper reference | Unlocked | 144 | CPU/GPU | `vm-run-002` invalid: TC failed before training from omitted canonical WOE; patch review; runtime/budget decision outstanding | P7C.3 |
| MLP-3 | replication baseline | Unlocked | 720 | CPU/GPU | `vm-run-002` invalid: TC failed before training from omitted canonical WOE; patch review; runtime/budget decision outstanding | P7C.3 |
| MLP-5 | replication baseline | Paper reference | Decision pending | 2,016 | GPU khuyến nghị | `vm-run-002` invalid: TC failed before training from omitted canonical WOE; no predictive exclusion; scope/budget decision outstanding | P7C.3 |
...
| P7C.3 | MLP-1/3/5 scope và budget decisions | **In progress:** historical `vm-run-002` is invalid (GMC 30/30 completed; TC 30/30 failed before training from omitted canonical WOE; independent JSON tuple/list `plan_mismatch`); immutable CPU-only plan remains unchanged and patch review/deployment precedes a new run | Chỉ feasibility execution được phê duyệt riêng; không dùng predictive metric để loại MLP-5 hoặc giảm grid |
...
P7C.3 chọn Branch B: `configs/protocols/p7c/p7c3_mlp_feasibility_plan.yaml` là kế hoạch engineering-feasibility bất biến, không phải final search-space/budget manifest. Historical `vm-run-002` is invalid and must not be resumed or reused: TC failed before training because the subprocess omitted canonical WOE, and a separate JSON round-trip bug caused `plan_mismatch`. Any later run requires a new output directory after review, commit and VM deployment. Kế hoạch khóa CPU-only sequential policy và cấm dùng kết quả feasibility/predictive để loại MLP-5 hoặc chọn/reduce grid hậu nghiệm.
