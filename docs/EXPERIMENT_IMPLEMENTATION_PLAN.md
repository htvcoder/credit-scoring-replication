...
replication, fold-level metrics và evidence so sánh thuộc Phase 7. Phase 7 đang in progress;
...
- P7C.4B.2: In Progress - Fresh target canary `p7c4b2d-target-rerun-01` đã operationally accepted: effective authorization trong scope canary, receipt, 4/4 `target_preflight` task, telemetry, completion marker và public validation đều hợp lệ. Canary vẫn non-scientific: coverage/projection không eligible và canonical execution chưa authorized. Target single-VM compute/outer-refit preflight và full scientific execution vẫn pending/NO-GO.
