...
Các blocker Phase 4 về metric validation đã được chốt trong Phase 4, và Phase 5–6 đã hoàn thành infrastructure/hardening. Theo source of truth hiện hành, Phase 0–6 là Completed, Phase 7 là In Progress: P7A/P7B đã completed và final CART-A 12-candidate manifest đã locked bằng evidence kỹ thuật non-publishable. P7C.1 đang thực hiện inventory/readiness planning, không chạy scientific execution; RF, XGBoost, MLP-1/3/5, CatBoost, TabNet và FT-Transformer vẫn cần final search-space/budget decision. Phase 0 acceptance có registry tập trung, checksum portable, target mapping đủ 6 dataset, data cards, dependency pin, reusable TH02 conversion và verifier chạy độc lập current working directory. HMEQ full và TH02 đã pass verification; không có scientific result nào được suy diễn từ smoke/reduced/dry-run/P7B artifact.
