...
Các blocker Phase 4 về metric validation đã được chốt trong Phase 4, và Phase 5–6 đã hoàn thành infrastructure/hardening. Theo source of truth hiện hành, Phase 0–6 là Completed, Phase 7 là In Progress: P7A đã completed và khóa protocol/manifest candidate; P7B đã implementation-ready và là bước next để đo feasibility non-publishable; P7B.2 chưa chạy. P7C vẫn Planned, chỉ có thể bắt đầu sau P7B closeout và khi final scientific search space được khóa. Phase 0 acceptance có registry tập trung, checksum portable, target mapping đủ 6 dataset, data cards, dependency pin, reusable TH02 conversion và verifier chạy độc lập current working directory. HMEQ full và TH02 đã pass verification; không có scientific result nào được suy diễn từ smoke/reduced/dry-run artifact.
