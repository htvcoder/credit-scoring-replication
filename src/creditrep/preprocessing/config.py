...
def parse_protocol_a_config(payload: dict[str, Any]) -> ProtocolAConfig:
    ...
    if sorted(set(woe_cfg) - allowed_woe):
        raise PreprocessingError(f"Unsupported protocol.woe keys: {sorted(set(woe_cfg) - allowed_woe)}.")
    if sorted(set(vif_cfg) - allowed_vif):
        raise PreprocessingError(f"Unsupported protocol.vif keys: {sorted(set(vif_cfg) - allowed_vif)}.")
    if sorted(set(scaling_cfg) - allowed_scaling):
        raise PreprocessingError(f"Unsupported protocol.scaling keys: {sorted(set(scaling_cfg) - allowed_scaling)}.")
    ...
    if not isinstance(smoothing, (int, float)) or isinstance(smoothing, bool) or smoothing <= 0:
        raise PreprocessingError(f"protocol.woe.smoothing must be > 0, got {smoothing!r}.")
    unknown_value = woe_cfg.get("unknown_value", 0.0)
    if not isinstance(unknown_value, (int, float)) or isinstance(unknown_value, bool):
    ...
    if not isinstance(threshold, (int, float)) or isinstance(threshold, bool) or threshold <= 1:
        raise PreprocessingError(f"protocol.vif.threshold must be > 1, got {threshold!r}.")
    minimum = vif_cfg.get("minimum_features_to_keep", 1)
    if not isinstance(minimum, int) or isinstance(minimum, bool) or minimum < 1:
        raise PreprocessingError("protocol.vif.minimum_features_to_keep must be an integer >= 1.")
