...
Website hiện mô tả phương pháp ở mức kế hoạch và trạng thái nghiệm thu kỹ thuật. P7C.3: Completed — canonical feasibility pilot accepted. Canonical `vm-run-003` xác nhận CPU feasibility PASS, memory feasibility PASS và execution stability PASS cho 60/60 fit. P7C.4A: Completed — benchmark plan ready for human review; P7C.4B.1 harness/readiness đang in progress theo partition contract human-approved, còn P7C.4B.2 canonical execution blocked. Benchmark chưa chạy, chưa khóa final MLP manifest và chưa chọn GPU. Đây là evidence engineering non-publishable, không phải kết quả hiệu năng dự báo hoặc kết quả khoa học cuối cùng. `vm-run-001` là historical/non-canonical và `vm-run-002` là historical invalid; không dùng chúng cho projection hoặc kết luận khoa học.
